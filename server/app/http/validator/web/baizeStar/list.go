...
import (
	...
	"goskeleton/app/http/controller/baizeStar"
...
func (t List) CheckParams(c *gin.Context) {
	if err := c.ShouldBind(&t); err != nil {
		errs := gin.H{
			"tips": "baizeStar 参数校验失败，参数不符合规定，page的值(>0)、limit的值(>0)",
			"err":  err.Error(),
		}
	...
	if ec := data_transfer.DataAddContext(t, consts.ValidatorPrefix, c); ec == nil {
		response.ErrorSystem(c, "baizeStar list 表单验证器json化失败", "")
	} else {
		(&baizeStar.BaizeStar{}).List(ec)
	}
}
